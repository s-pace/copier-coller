...
import ModelHit, { ModelHitSchema } from "@/components/ModelHit";
import {
  Drawer,
  DrawerContent,
  DrawerHeader,
...
function ModelPage() {
  ...
  const [selectedModel, setSelectedModel] = useState<ModelHitSchema | null>(
    null,
  );
  ...
  const HitComponent = ({ hit }: { hit: ModelHitSchema }) => (
    <ModelHit
      key={hit.id}
      order={{
        ...hit,
  ...
  return (
    ...
    >
      <InstantSearch searchClient={searchClient} indexName="prod.art">
        <Configure filters="type:Request" />
